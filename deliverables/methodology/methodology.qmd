...
In our attempt to classify Stack Overflow questions as "good" or "bad" based on their score, we will use a variety of machine learning techniques. We will use logistic regression, (extreme) gradient boosted trees, and SVC. We expect gradient boosting and SVC to achieve a better classification error than logistic regression, however as we are interested in inference, we include it as it is far more interpretable than the other two. There is much potential for improvement, particularly with how we select our features. We expect that interaction features may play a more important role than raw features - for example, code to text ratio may be a more important metric than simply the number of words in a post - and it can be very difficult to identify these interaction features.
